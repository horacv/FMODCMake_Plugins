...
namespace ans_gain
{
    ...
    void ANS_DSP_State::set_gain_db(const float db)
    {
        ...
        const float ramp_time_ms = m_data.m_defaults.m_smoothing_time_ms;
        const unsigned int samplerate = m_data.m_defaults.m_samplerate;
        m_data.m_gain_lin_target = ans_dsp::db_to_linear(std::clamp(db, gain_db_min, gain_db_max));
        m_data.m_samples_to_target = ans_dsp::ms_to_samples(ramp_time_ms, static_cast<float>(samplerate));
    }
    ...
    {
        // This implementation is based on this example:
        // 'FMOD Programmers API/api/core/examples/plugins/fmod_gain.cpp'
