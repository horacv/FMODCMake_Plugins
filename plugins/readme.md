...
- **Per-instance state** is allocated via `FMOD_DSP_ALLOC`/`FMOD_DSP_FREE`
  (see `alloc_dsp_state`/`get_dsp_state` in `libs/ans_fmod_dsp`), not raw
  `new`/`delete`, and attached to `FMOD_DSP_STATE::plugindata`.
- **Shared helpers** for parameter descriptors and DSP state plumbing live in
  `libs/ans_fmod_dsp`; general-purpose DSP math (no FMOD dependency) lives in
